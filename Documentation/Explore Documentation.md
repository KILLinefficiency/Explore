...
<br>
#### Introduction
Explore is a REPL-type data manager for small scale purposes. Explore has a CLI but also comes with a Python Package which allows programmers to integrate Explore into their Python Programs.

Explore is distributed under the [GNU General Public License v3.0](https://choosealicense.com/licenses/gpl-3.0/).

Explore is currently at version 1.0 titled *Prometheus*.

This documentation covers Explore on a **Linux Distribution**.

#### Getting Explore
Explore is available in source code form only at the official Explore repository can be cloned from [GitHub](htttps://www.github.com/KILLinefficiency/Explore).
You can either clone the repository directly from the GitHub page or by using the terminal.

**Cloning via terminal:**

Make sure you have a newer version of ``git``, ``python3`` and ``pip3`` installed on your machine.
...
Explore is made up of five files:

 * ``cipher.py`` contains the code for the security layer in Explore.

 * ``login.py`` contains the code for the login system in Explore.

 * ``lib.py`` contains a few Explore standard functions required for it's functioning.

 * ``shell.py`` is an important file which contains the code responsible for the executions of commands. ``shell.py`` uses all the standard functions defined in ``lib.py``.

 * ``Explore.py`` is the main file run by the Python interpreter. It just launches the ``shell.py`` file.

Apart from this files, the repository also conatains file for Explore's Python module.
...
Explore uses two memory locations to store it's data. The primary being the **Mess** and the secondary being the **Cluster**.
...
Reference for a data item in the **Mess** is it's *Position* preceded by ``x_``
```
x_<position-of-the-data-item-in-the-Mess>
```

Reference for a data item in the **Cluster** is it's *Key* preceded by ``y_``
```
...
2. pi
```
``push`` can take alphabetic data conataning spaces too.
```
...
```
The user can set the values from the **Mess** or from within the **Cluster**
Like,
```
:) > push num 3.14
:) > set value alpha pi
...
g : 10
```
The *Value* can also be changed by referring it from within the **Cluster** or from the **Mess**.
...
#### *10. count*
``count`` displays the number of data items in the **Mess** or the **Cluster** as specified by the user.
```
count mess
```
```
...
<br>
#### *11. get*
``get`` accesses data from the **Mess** or the **Cluster**
```
get mess <position>
```
```
...
``disp`` can also be used to display data directly from the **Mess** or from the **Cluster**.
```
disp x_<position>
```
...
<br>
#### *13. clean*
``clean`` is useful when the user wants to remove all the data items from the **Mess** or the **Cluster**.
```
clean mess
```
```
...
30
```
Values from the **Mess** and the **Cluster** can also be referred in ``calc``.
...
Using ``w``:
```
:) > push alpha Prometheus
:) > set codename alpha Prometheus
:) > getmess
1. Prometheus
:) > getcluster
Key : Value
...
:) > csv 1 /home/me/Desktop/data.csv
...
<br>
#### *21. find*
``find`` searches for the entered data item. ``find`` can search data items from the **Mess** and the *Keys* and the *Values* from the **Cluster**.
```
...
```
pip3 uninstall explore_package_KILLinefficiency
```
<hr>
<br>
#### Hey There!
